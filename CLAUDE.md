...
1. **做他自己的产品或 idea**——两条走法,先问是不是"LLM + 网页"形态的想法:
   - 是 → **借壳**:`pnpm new-demo <他的slug>` 直接在本仓库里生成三件套,LLM 调用/限流/手机适配全现成,他只写 config.ts 的 prompt 和一层薄 UI,30 分钟能出第一版。这是短时间窗口里的最快路径,主动推荐它
   - 不是(CLI/机器人/别的框架)→ 正常开工;`.env.local` 里的中转站是标准 OpenAI-compatible,任何框架都能直接调
