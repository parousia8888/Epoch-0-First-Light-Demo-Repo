...
| 动作 | 说明 |
|---|---|
| 打开 **`/status`** | 健康自检页(不在首页索引里):环境变量、中转站连通性、EVENT_MODE 一屏全绿才开场 |
| 打开 **`/qr`** 打印 | 首页 + 10 个 demo 的二维码,一页打完剪开贴桌。**务必在正式域名下打开再打印** |
...
| 换单个 demo 的模型 | 同上文件 → 改 `model:` 字段,中转站支持的名字都行 |
| **全场换模型**(中转站没这个模型/集体太慢) | `.env.local` 或 Vercel 环境变量加 `LLM_MODEL_OVERRIDE=模型名`,一处覆盖全场;清空即恢复 |
...
## 散场后关站 SOP(活动当晚 10 分钟)

1. **换 key**:中转站后台作废活动用的 key,签一把新的 → 填进 Vercel 环境变量(防止 QR 流出去被刷)
2. **降档**:Vercel 环境变量 `EVENT_MODE` 改成 `false` → Redeploy(拍照类 demo 下线,文本类保留,成本可控)
3. **留观两周**:站保持可访问,参会者回家还能点开转发;两周后看情况决定关站或转常驻
4. (可选)把 demo pack 发到 Epoch 0 站上当作品页,给"带走的人"一个社区内入口
